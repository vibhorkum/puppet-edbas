...
define edbas::server::grant (
  $role,
  $db,
  $privilege        = undef,
  $object_type      = 'database',
  $object_name      = undef,
  $psql_db          = $edbas::server::default_database,
  ...
  case $_object_type {
    ...
    'ALL SEQUENCES IN SCHEMA': {
      ...
      # single row, which satisfies the `unless` parameter of edbas_psql.

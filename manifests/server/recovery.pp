...
define edbas::server::recovery(
  ...
  $target                         = $edbas::server::recovery_conf_path
) {

  if $edbas::server::manage_recovery_conf == false {
    fail('edbas::server::manage_recovery_conf has been disabled, so this resource is now unused and redundant, either enable that option or remove this resource from your manifests')
  } else {
    if($restore_command == undef and $archive_cleanup_command == undef and $recovery_end_command == undef
      ...
      fail('edbas::server::recovery use this resource but do not pass a parameter will avoid creating the recovery.conf, because it makes no sense.')
    }

    # Create the recovery.conf content
    concat::fragment { 'recovery.conf':
      target  => $target,
      content => template('edbas/recovery.conf'),
    }
  }
}

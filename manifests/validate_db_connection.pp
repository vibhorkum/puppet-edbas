# This type validates that a successful edbas connection can be established
...
define edbas::validate_db_connection(
  $database_host     = undef,
  $database_name     = undef,
  ...
  $cmd_dbname = $database_name ? {
    undef   => "--dbname ${edbas::params::default_database} ",
  ...
  if($create_db_first) {
   Edbas::Server::Database<|title == $database_name|> -> Exec[$exec_name]
  #  edbas::server::database<|title == $database_name|> -> Exec[$exec_name]
  }
}
